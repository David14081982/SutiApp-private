...
Toda H que modifique realmente `AssetRepository`, `DocumentWorkflowRepository`, `app_assets`, `private_assets`, políticas Storage, URLs firmadas, lógica de `sw.js`, el viewer compartido, routing/app shell compartido, autenticación global o repositories/helpers compartidos entre múltiples superficies debe ejecutar `scripts/test-global-image-regression-production-live.js` contra el build local y GitHub Pages. `app/bundle.js` y los cachebusters asociados, cuando cambian exclusivamente por regeneración desde fuentes de un módulo focal ya identificado, se clasifican `GENERATED_ARTIFACT` y no convierten la H en global ni disparan por sí solos esa regresión. En ese caso se ejecutan sólo pruebas focales, dependencias directas y `post-change-verification`; no se pide autorización owner por esta decisión técnica reversible. Cuando la regresión global sí aplique, no puede declararse `PASS` si sello/Login, foto de perfil, Admin Afiliados, documentos imagen/PDF, Membership, Préstamo, catálogo de programas/galería, Marketplace, fullscreen, refresh o la comparación con/sin service worker no quedan `PASS` con assets legítimos y sin reescritura de datos.
