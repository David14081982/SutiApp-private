...
(function(){
  ...
  function flush(){
    ...
    const current=(async()=>{
      ...
      settled.forEach((r,i)=>{
        ...
        else if(key==='banners')ads=v.filter(x=>x.placement==='marketplace');
      });
  ...
  const projectAd=a=>({id:a.id,empresa:a.title||'',etiqueta:a.description||'',link:a.action_url||'',hue:a.accent_hue==null?215:a.accent_hue,visible:a.enabled,order:a.sort_order,image_asset_id:a.image_asset_id||null,image_url:window.AssetRepository.publicUrl(a.image_asset)||null,audience:{mode:a.audience_mode||'all',sindicatos:toLabels('union',a.union_codes),niveles:toLabels('employment_category',a.employment_category_codes),generos:toLabels('gender',a.gender_codes),cargos:toLabels('tag',a.tag_codes)}});
  ...
  const adRow=a=>{const aud=a.audience||{},segment=aud.mode==='segment';return{id:a.id||undefined,placement:'marketplace',title:String(a.empresa||'').trim(),description:String(a.etiqueta||'').trim(),action_url:String(a.link||'').trim()||null,enabled:a.visible!==false,sort_order:a.id?a.order:undefined,image_asset_id:a.image_asset_id,accent_hue:a.hue==null?null:Number(a.hue),audience_mode:aud.mode||'all',union_codes:segment?toCodes('union',aud.sindicatos):[],employment_category_codes:segment?toCodes('employment_category',aud.niveles):[],gender_codes:segment?toCodes('gender',aud.generos):[],tag_codes:segment?toCodes('tag',aud.cargos):[]};};
  store.anunciosAll=()=>ads.map(projectAd);store.getAnuncio=id=>store.anunciosAll().find(x=>x.id===id);store.anuncioVisibleFor=(a,v)=>a.visible&&store.audienceMatch(a,v||store.viewer());store.anunciosLive=v=>store.anunciosAll().filter(x=>store.anuncioVisibleFor(x,v));
  store.blankAnuncio=()=>({id:null,empresa:'',etiqueta:'',link:'',hue:215,visible:true,order:ads.length+1,image_asset_id:null,image_url:null,audience:{mode:'all',sindicatos:[],niveles:[],generos:[],cargos:[]}});
  store.saveAnuncio=async a=>{if(!a.image_asset_id)throw new Error('AD_IMAGE_REQUIRED');const saved=await window.AdminRepository.saveManaged('banners',adRow(a));await load();return saved;};
  store.toggleAnuncio=id=>{const a=store.getAnuncio(id);if(a)window.AdminRepository.saveManaged('banners',{id,enabled:!a.visible}).then(load).catch(fail);};
