...
(function () {
  ...
  function DesktopFinancialWorkbench({ app, onCount, initialAffiliateId }) {
    ...
    const actionOptions = React.useMemo(() => { if (!detail || !app.admin.has('program_requests.write')) return []; const productPayment=detail.program_id!=='prestamo';const options = [{ id: 'note', label: 'Guardar observación' }]; if (['requires_financial_processing', 'submitted'].includes(detail.status)) options.unshift({ id: 'review', label: 'Marcar en revisión' }); if (['requires_financial_processing', 'submitted', 'in_review'].includes(detail.status)) options.push({ id: productPayment?'approveProduct':'approve', label: productPayment?'Aprobar en Supabase':'Aprobar y enviar a gestión' }, { id: 'reject', label: 'Rechazar solicitud' }, { id: 'cancel', label: 'Cancelar solicitud' }); if (!productPayment&&detail.status === 'approved' && ['ready_for_handoff', 'failed'].includes(detail.financial_processing_status)) options.unshift({ id: 'handoff', label: detail.financial_processing_status === 'failed' ? 'Reintentar envío a gestión' : 'Enviar a gestión' }); return options; }, [detail, app]);
