...
(function () {
  ...
  function timelineEvents(detail) {
    ...
    if (approvedAt) events.push({ at: approvedAt, title: 'Financiamiento aprobado', text: 'Condiciones aprobadas guardadas en snapshot inmutable.' });
  ...
  function DesktopFinancialWorkbench({ app, onCount, initialAffiliateId }) {
    ...
    const save = async (advance) => { if (!detail || !action || busy) return; if (action === 'note' && actionNote.trim().length < 3) { setFeedback({ tone: 'error', text: 'Escribe una observación de al menos 3 caracteres.' }); return; } if (action === 'reject' && actionNote.trim().length < 3) { setFeedback({ tone: 'error', text: 'Indica el motivo del rechazo.' }); return; }
      const confirmations = { approve: '¿Aprobar este financiamiento? El backend revalidará las condiciones financieras vigentes.', handoff: '¿Enviar esta solicitud aprobada a la gestión financiera?', reject: '¿Rechazar esta solicitud financiera?' };
      ...
      try { const note = [detail.notes, actionNote.trim()].filter(Boolean).join('\n'); if (action === 'review') await window.ProgramRequestRepository.update(currentId, 'in_review', note); else if (action === 'reject') await window.ProgramRequestRepository.update(currentId, 'rejected', note); else if (action === 'note') await window.ProgramRequestRepository.update(currentId, detail.status, note); else if (action === 'approve') await window.FinancialLegacyRepository.approveRequest(currentId); else if (action === 'handoff') await window.FinancialLegacyRepository.handoffRequest(currentId);
        const refreshed = await load(true), verified = refreshed.find((row) => row.id === currentId); const valid = verified && (action === 'review' ? verified.status === 'in_review' : action === 'reject' ? verified.status === 'rejected' : action === 'approve' ? verified.status === 'approved' && ['ready_for_handoff', 'in_progress', 'handed_off'].includes(verified.financial_processing_status) : action === 'handoff' ? verified.financial_processing_status === 'handed_off' : true); if (!valid) throw new Error('FINANCIAL_ACTION_READBACK_FAILED');
        setFeedback({ tone: 'success', text: '✓ Actualizado y verificado' }); setRowFeedback((all) => Object.assign({}, all, { [currentId]: 'success' })); setActionNote(''); if (advance && nextId !== currentId) setSelectedId(nextId); else setDetailNonce((value) => value + 1);
    ...
    const prepareDocument = async (document) => { setDocumentViews((all) => Object.assign({}, all, { [document.id]: { phase: 'loading' } })); try { const preview = await window.DocumentWorkflowRepository.reviewPreview(document.affiliate_document_id); if (!preview.signedUrl) throw new Error('PREVIEW_UNAVAILABLE'); setDocumentViews((all) => Object.assign({}, all, { [document.id]: { phase: 'ready', url: preview.signedUrl } })); } catch (_) { setDocumentViews((all) => Object.assign({}, all, { [document.id]: { phase: 'error' } })); } };
    ...
    const renderDetail = () => { if (detailPhase === 'loading') return h('div', { className: 'finwb-empty' }, h(I, { name: 'clock', size: 28 }), 'Cargando detalle autorizado…'); if (detailPhase === 'error') return h('div', { className: 'finwb-empty' }, h(I, { name: 'warning', size: 28 }), 'No fue posible cargar el detalle.', h('button', { onClick: () => setDetailNonce((value) => value + 1) }, 'Reintentar')); if (!detail) return h('div', { className: 'finwb-empty' }, 'Selecciona una solicitud para revisar su expediente.'); const submission = snapshotResult(detail, false), approval = snapshotResult(detail, true), events = timelineEvents(detail);
      return h(React.Fragment, null,
        ...
        h('div', { className: 'finwb-detail-scroll' },
          ...
          h('section', { className: 'finwb-card', 'data-financial-documents': 'true' }, h('h3', null, h(I, { name: 'doc', size: 17, stroke: 2 }), 'Documentos'), !detail.documents_available ? h('div', { className: 'finwb-snapshot-note' }, 'No fue posible consultar la relación documental autorizada.') : !(detail.request_documents || []).length ? h('div', { className: 'finwb-sub' }, 'Sin documentos vinculados a esta solicitud.') : detail.request_documents.map((document) => { const view = documentViews[document.id] || {}; return h('div', { className: 'finwb-doc', key: document.id }, h(I, { name: 'doc', size: 18, stroke: 2 }), h('div', { className: 'finwb-doc-main' }, h('div', { className: 'finwb-person' }, document.document_type && document.document_type.label || 'Documento'), h('div', { className: 'finwb-sub' }, 'Estado al enviar: ' + (document.status_at_submission || 'No disponible'))), view.phase === 'ready' ? h('a', { href: view.url, target: '_blank', rel: 'noopener noreferrer' }, 'Abrir') : h('button', { type: 'button', disabled: view.phase === 'loading', onClick: () => prepareDocument(document) }, view.phase === 'loading' ? 'Autorizando…' : view.phase === 'error' ? 'Reintentar' : 'Preparar vista')); })),
          ...
          h('section', { className: 'finwb-card', 'data-financial-timeline': 'true' }, h('h3', null, h(I, { name: 'clock', size: 17, stroke: 2 }), 'Timeline'), h('div', { className: 'finwb-timeline' }, events.map((event, eventIndex) => h('div', { className: 'finwb-event', key: event.title + eventIndex }, h('span', { className: 'finwb-event-dot' }), h('div', null, h('strong', null, event.title), h('p', null, dateValue(event.at) + ' · ' + event.text))))))),
        h('div', { className: 'finwb-actionbar', 'data-financial-safe-action-bar': 'true' }, actionOptions.length ? h(React.Fragment, null, h('div', { className: 'finwb-action-grid' }, h('select', { className: 'finwb-action-select', value: action, disabled: busy, onChange: (event) => setAction(event.target.value), 'aria-label': 'Acción financiera permitida' }, actionOptions.map((item) => h('option', { key: item.id, value: item.id }, item.label))), h('div', null, h('span', { className: 'finwb-sub' }, 'Etapa actual'), h('strong', { style: { fontSize: 11.5 } }, stageMeta(detail.financial_processing_status).label))), h('textarea', { className: 'finwb-note', value: actionNote, disabled: busy, onChange: (event) => setActionNote(event.target.value), placeholder: action === 'reject' ? 'Motivo obligatorio del rechazo' : 'Observación (obligatoria al guardar observación)', 'aria-label': 'Observación de la acción' }), h('div', { className: 'finwb-buttons' }, h('button', { className: 'finwb-secondary', disabled: index <= 0 || busy, onClick: () => move(-1) }, 'Anterior'), h('button', { className: 'finwb-secondary', disabled: index < 0 || index >= visible.length - 1 || busy, onClick: () => move(1) }, 'Siguiente'), h('button', { className: 'finwb-primary', disabled: busy || !action, onClick: () => save(true) }, busy ? 'Guardando…' : 'Guardar y siguiente')), feedback && h('div', { className: 'finwb-feedback', 'data-financial-action-feedback': feedback.tone, 'data-tone': feedback.tone }, feedback.text)) : h('div', { className: 'finwb-sub' }, app.admin.has('program_requests.write') ? 'No hay transiciones disponibles para este estado.' : 'Consulta autorizada; las acciones requieren permiso de escritura.')));
    };
        ...
        React.createElement('div', { style: { background: 'var(--surface)', borderRadius: 16, padding: 15, boxShadow: 'var(--neo-sm)', marginBottom: 14 } },
          ...
          React.createElement('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8 } },
            window.FINANZAS.ESTADOS.map((e) => React.createElement('button', {
              key: e.id, disabled:e.id==='depositada'||!app.admin.has('program_requests.write'), title:e.id==='depositada'?'La confirmación del depósito se realiza por separado':'', onClick: async () => {if(e.id==='aprobada'&&!window.confirm('¿Aprobar este financiamiento? El backend revalidará las condiciones financieras vigentes.'))return;try{await store.setEstado(r.id,e.id);app.toast&&app.toast('Estado actualizado');}catch(_){app.toast&&app.toast(e.id==='depositada'?'El depósito se confirma en el sistema financiero':'No se pudo actualizar');}},
