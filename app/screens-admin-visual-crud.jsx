    ...
    return React.createElement('div',{'data-h009-editor':kind,style:{position:'absolute',inset:0,zIndex:30,background:'var(--bg)',display:'flex',flexDirection:'column'}},
      ...
      React.createElement('div',{className:'su-app-scroll',style:{padding:16,flex:1}},
        ...
        kind==='education'&&form.resource_kind==='education'&&React.createElement('div',null,
          React.createElement(window.CommercialCategoryField,{label:'Categoría de la institución',value:form.public_details?.category_label||'',onChange:value=>setForm(old=>({...old,public_details:{...old.public_details,category_label:value}}))}),
        ...
        kind==='companies'&&React.createElement(window.CommercialCategoryField,{label:'Categoría comercial',value:form.category_raw||'',onChange:value=>setForm(old=>({...old,category_raw:value}))}),
        kind==='companies'&&[['Ubicación','address_raw'],['Teléfono','phone_raw'],['WhatsApp','whatsapp_raw'],['Correo','email_raw'],['Sitio web','website_url']].map(([label,key])=>field(label,key,form,setForm)),
