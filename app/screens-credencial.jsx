...
(function () {
  ...
  function QRCode({ size = 200, fg = '#1c1518', value = '' }) {
    if (!value || typeof window.qrcode !== 'function') return React.createElement('div', { role: 'status', style: { width: size, height: size, display: 'grid', placeItems: 'center', color: 'var(--ink-3)', fontSize: 'var(--text-12, 12px)' } }, 'Generando código…');
    ...
    return React.createElement('div', { className: 'su-route', 'data-affiliate-id': u.id, style: { paddingBottom: 18 } },
      ...
      React.createElement('div', { style: { padding: '22px 20px 0' } },
        ...
        sections.map((sec) => React.createElement('div', { key: sec.g, style: { marginTop: 16 } },
          React.createElement('div', { style: { fontSize: 'var(--text-12, 12px)', fontWeight: 900, color: 'var(--guinda)', letterSpacing: '.04em', textTransform: 'uppercase', margin: '0 4px 8px' } }, sec.g),
      ...
      React.createElement('div', { style: { padding: '22px 20px 0' } },
        ...
        React.createElement('div', { style: { background: 'var(--surface)', borderRadius: 18, padding: 15, boxShadow: 'var(--neo-sm)', display: 'flex', alignItems: 'center', gap: 12 } },
          ...
          React.createElement('div', { style: { flex: 1 } }, React.createElement('div', { style: { fontSize: 'var(--text-14, 14px)', fontWeight: 800 } }, 'Expediente digital'), React.createElement('div', { style: { fontSize: 'var(--text-12, 12px)', color: 'var(--ink-3)', fontWeight: 600, marginTop: 2 } }, 'Adjunta o toma una foto; los verificados permanecen protegidos.')),
      ...
      React.createElement('div', { style: { padding: '16px 20px 0', display: 'flex', gap: 10, alignItems: 'center', color: 'var(--ink-3)' } },
        ...
        React.createElement('span', { style: { fontSize: 'var(--text-12-5, 12.5px)', fontWeight: 600 } }, 'Identidad cargada desde Supabase')));
  }

  function CardFront({ u }) {
    return React.createElement('div', { className: 'su-credential-face', style: { width: '100%', height: '100%', borderRadius: 26, overflow: 'hidden', position: 'relative', background: 'linear-gradient(150deg, var(--guinda) 0%, #6a001b 60%, #45000f 100%)', color: '#fff', boxShadow: '0 24px 60px -18px rgba(145,0,34,.65)', padding: 24, display: 'flex', flexDirection: 'column' } },
      ...
      React.createElement('div', { style: { marginTop: 16 } },
        React.createElement('div', { 'data-affiliate-field': 'credential-name', style: { fontSize: 'var(--text-22, 22px)', fontWeight: 800, letterSpacing: '-.01em' } }, u.name),
        React.createElement('div', { style: { fontSize: 'var(--text-13, 13px)', opacity: .82, fontWeight: 600, marginTop: 2 } }, u.status + ' · ' + u.affiliation)),
      React.createElement('div', { style: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 16 } },
        React.createElement('div', null,
          React.createElement('div', { style: { fontSize: 'var(--text-10, 10px)', opacity: .7, fontWeight: 600, letterSpacing: '.08em' } }, 'No. AFILIADO'),
          React.createElement('div', { 'data-affiliate-field': 'credential-control', style: { fontSize: 'var(--text-18, 18px)', fontWeight: 700, fontFamily: 'var(--mono)' } }, u.numeroControl)),
        React.createElement('div', { style: { textAlign: 'right' } },
          React.createElement('div', { style: { fontSize: 'var(--text-10, 10px)', opacity: .7, fontWeight: 600, letterSpacing: '.08em' } }, 'ESTATUS'),
          React.createElement('div', { 'data-affiliate-field': 'credential-status', style: { fontSize: 'var(--text-18, 18px)', fontWeight: 700, fontFamily: 'var(--mono)' } }, u.status))));
  }
  ...
  function BankAccounts({ app }) {
    ...
    return React.createElement('div',{'data-banking-authority':'supabase','data-banking-phase':phase,style:{padding:'22px 20px 0'}},
      ...
      phase==='error'&&React.createElement('div',{role:'alert',style:{color:'#A32921',fontSize: 'var(--text-12, 12px)',fontWeight:700}},error,' ',React.createElement('button',{onClick:load},'Reintentar')),
      phase==='ready'&&React.createElement('div',{style:{display:'flex',flexDirection:'column',gap:10}},
        rows.map((r)=>React.createElement('div',{key:r.id,'data-bank-account-id':r.id,'data-bank-status':r.data_status,style:{background:'var(--surface)',borderRadius:16,padding:14,boxShadow:'var(--neo-sm)',color:'var(--ink)'}},
          React.createElement('button',{onClick:()=>setEditing(Object.assign({},r)),style:{width:'100%',border:'none',padding:0,textAlign:'left',background:'transparent',color:'inherit'}},
            React.createElement('div',{style:{display:'flex',justifyContent:'space-between',gap:10}},React.createElement('b',null,r.bank_name||'Banco pendiente'),r.is_primary&&React.createElement('span',{style:{fontSize: 'var(--text-10, 10px)',color:'var(--guinda)',fontWeight:900}},'PRINCIPAL')),
            React.createElement('div',{'data-bank-masked':'true',style:{fontFamily:'var(--mono)',fontSize: 'var(--text-12, 12px)',color:'var(--ink-2)',marginTop:5}},r.maskedClabe||r.maskedAccount||'Datos pendientes'),
            React.createElement('div',r.data_status==='INCOMPLETE_HISTORICAL_DATA'?{'data-bank-incomplete':'true',style:{fontSize: 'var(--text-11-5, 11.5px)',color:'var(--guinda)',fontWeight:800,marginTop:3}}:{style:{fontSize: 'var(--text-11-5, 11.5px)',color:'var(--ink-3)',fontWeight:600,marginTop:3}},r.data_status==='INCOMPLETE_HISTORICAL_DATA'?'Completa tus datos bancarios':r.account_holder)),
  ...
  function BankEditor({value,onSaved,app}) {
    ...
    return React.createElement('div',null,
      ['account_holder','bank_name','clabe','account_number'].map((k)=>React.createElement('label',{key:k,style:{display:'block',fontSize: 'var(--text-12, 12px)',fontWeight:800,marginTop:12}},({account_holder:'Titular de la cuenta',bank_name:'Banco',clabe:'CLABE interbancaria',account_number:'Número de cuenta'})[k],React.createElement('input',{value:d[k]||'',inputMode:['clabe','account_number'].includes(k)?'numeric':undefined,onChange:(e)=>set(k,e.target.value),style:input}))),
      React.createElement('div',{style:{fontSize: 'var(--text-11-5, 11.5px)',color:'var(--ink-3)',marginTop:12}},'Usa solo dígitos. La CLABE es opcional y debe tener 18 dígitos; no se acepta notación científica.'),
