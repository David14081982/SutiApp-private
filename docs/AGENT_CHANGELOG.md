...
- El repository cliente conserva el enlace exacto por UUID y respeta la certificación fail-closed del backend sin volver a imponer el email histórico; `v=5` y app-shell `v151` expulsan la copia anterior. Forward/recovery, seguridad/RLS, sesión focal, publicación y artefacto pasaron; la regresión protegida local/publicada quedó bloqueada antes de evaluar superficies porque el fixture preexistente `H005_TEST` devuelve `400 invalid_credentials` y no fue alterado. Evidencia: `docs/qa/H-AFFILIATES-CSV-AUTH-LINK-REPAIR-001-EVIDENCE.md`; ADR-104; INV-210.
