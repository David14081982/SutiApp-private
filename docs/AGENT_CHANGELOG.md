...
Trigger libera la ficha que queda única tras corregir un duplicado. Migración aditiva
...
aislado con edición/clamp y responsive. Sin cambios backend/legacy.
Publicado por OWNER: 29753c9, bundle 291 / worker 225; Actions 36192441028 SUCCESS.
Paridad de archivos y Chrome focal en sutiapp.com PASS; sin escrituras productivas.
...
22 fondos configurados cubiertos; 67 condiciones del Excel coinciden. Otros 11
no figuran en el archivo y quedan pendientes de alcance/condiciones del propietario.
...
- El lote `8ebd3cd8-1f57-5054-953d-c2a7fe12af66` guardó snapshot de 21 afiliados y una auditoría por movimiento. No cambió `auth.users`, correos históricos, nombres, controles, perfiles, filas ambiguas ni legacy.
- Los resolvedores admiten cada movimiento sólo con evidencia activa exacta de UUID, correo Auth confirmado, afiliado y control. Los 84 vínculos pasaron read-back; 76 corresponden al CSV y quedan 0 cruces determinísticos. `cosaf@hotmail.com → 1536 → PRECIADO RAMIREZ XOCHITL NOHEMI` pasó y 224761 quedó sin ese UUID.
- El repository cliente conserva el enlace exacto por UUID y respeta la certificación fail-closed del backend sin volver a imponer el email histórico; `v=5` y app-shell `v151` expulsan la copia anterior. El cierre rotó sólo la contraseña QA `H005_TEST` y sincronizó su secreto local ignorado: login `ACTIVE`, mismo afiliado/UUID/email/asignación y 84 vínculos intactos. La única ejecución posterior de la regresión terminó por `NODE_HEAP_OOM` antes de emitir superficies; clasificada `OUT_OF_SCOPE / PREEXISTING`, sin retry. Evidencia: `docs/qa/H-AFFILIATES-CSV-AUTH-LINK-REPAIR-001-EVIDENCE.md`; ADR-104; INV-210.
...
- Chrome contra el build corregido y, después, contra GitHub Pages productivo pasó login válido, inválido, indisponibilidad controlada, refresh, sesión, logout y 390/430/1280 sin errores de página ni escrituras de negocio. Workflow `33584782213` desplegó `053d49a8ba57436b51c094ea6c1b01e789858a20`; su gate Auth, build y deploy finalizaron `success`.
...
Tests: PASS — 80/80 static, real repository, production backend and public Pages matrix
...
Owner-authorized after H01-H04 PASS. One measured evidence index and one conditional endpoint delegate to the exact certified getter; no financial/history/rule changes. Ten full real-case equivalence checks, 24 table hashes, backend security/invalidation and recovery pass. Memory-only projection validated on each read, identity/session/context/write/error invalidation. Isolated two-chunk release, global local/production regression, complete Registry suite and public byte verification PASS. Financial payload/UI unchanged; 24 tables identical. Evidence: [H05 PASS](qa/evidence/disk-io-remediation/H05/VERIFICATION.md). No H06.

## 2026-09-07 — Disk I/O H06: demanda de catálogo y recursos privados — PASS

Solicitud explícita H06; H01–H05 PASS. Catálogo por programa, firmas por visibilidad y previews por clave estable, con deduplicación sólo en vuelo y separación de actor/sesión/contexto. Sin modificación backend/RLS/TTL/buckets, reglas financieras, productos, históricos ni diseño. Navegación publicada: 134→3 productos, 268→27 vínculos, 268→6 objetos firmados, 57.003.176→9.478.048 bytes. Quince comparaciones completas y UI Admin iguales; 192 casos RLS; trece suites focales; regresión global local/GitHub Pages y Registry PASS. Documentos históricos intactos; una inserción concurrente de otro actor documentada. Runtime 009885b, bundle223/worker170, workflow34182408615 SUCCESS; 21 archivos públicos verificados. Rollback preparado y trabajo previo preservado en checkout aislado. [Verificación](qa/evidence/disk-io-remediation/H06/VERIFICATION.md), [revisión](qa/evidence/disk-io-remediation/H06/ARCHITECT_REVIEW.md). H07 no iniciada.

## 2026-09-08 — Disk I/O H07 — N+1 implementado; gate público BLOCKED

Solicitud explícita H07 y precondiciones H01–H06 PASS. Publicación aislada 7cecf8a: portal batch FK/RLS 39→7 en store válido aislado; lector real 34→2, transferencia instrumentada87.405→10.609 B. Edge data-exports v13 + RPC service-only limitada1000:123 lookups Auth→1 lote; CSV completo/XLSX campo por campo idénticos de954 afiliados. Históricos/identidades/headers/permisos/audit intactos. Miniaturas Admin demandadas,8→3 iniciales en viewport aislado, scroll completo equivalente. Reordenamientos con commits parciales no agrupados. Recovery disponible;17 suites focales, Registry y regresión global local PASS. Backend253 funciones anteriores/228 policies y hashes maestros/Auth iguales. Workflow SUCCESS y21 archivos públicos exactos.

H07 STATUS: BLOCKED: regresión global pública detenida por Auth400 invalid_credentials de la cuenta controlada; diagnóstico independiente confirma. No se cambian credenciales ni se omite el gate. Se pidió actualizar supabase.env local sin enviar contraseña al chat. Evidencia: docs/qa/evidence/disk-io-remediation/H07/VERIFICATION.md y ARCHITECT_REVIEW.md. H08 no iniciado. Cambios financieros previos del workspace no publicados.
...
Cierre Votaciones: PASS / APPROVED; commit funcional 5aecd33, GitHub Pages SUCCESS, bundle SHA verificada y global-production PASS. Evidencia final: docs/qa/evidence/voting-20260915/result.md. Sin pruebas de carga ni datos demo persistidos.

## H-SUTIAPP-VOTACIONES-VER-003

Admin «VER» en Votaciones (a pedido del propietario, para pruebas): lista de votos por persona con borrar un voto o todos los de una persona. Migración aditiva 20260917000200_voting_vote_admin (list_voting_votes, delete_voting_votes; el trigger de inmutabilidad solo deja pasar DELETE dentro de la RPC, UPDATE sigue prohibido). Permisos: ver = export_identified_votes; borrar = además delete. Por decisión del propietario los borrados NO se registran en admin_audit_log. Bundle: solo trozos voting-repository.js y screens-voting.jsx (voting-ver-20260917-001, 123 chunks intactos). Matriz SQL con ROLLBACK PASS a la primera, apply + verify PASS, navegador aislado PASS (15 checks). Recuperación: supabase/recovery/20260917000200_voting_vote_admin.sql + tag restore/pre-voting-ver-20260917. Evidencia: docs/qa/evidence/voting-ver-20260917.
