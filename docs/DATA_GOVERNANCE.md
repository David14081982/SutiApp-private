...
`financial_session_snapshots` es la excepción temporal autorizada por ADR-043. Origen: Google `Criterios de fondos` después de aplicar el perfil autoritativo Supabase y `loan_term_policy`. Propietario/escritor: Edge `financial-legacy` con service role. Lectores: exclusivamente la misma Edge. Retención lógica: máximo 15 minutos; `expires_at` vencido siempre es inválido aunque cleanup aún no borre la fila.
