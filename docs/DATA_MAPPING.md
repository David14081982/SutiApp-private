...
| 2 | Perfil/credencial/depósito | proyección in-memory; foto privada firmada; `BankAccountRepository` | Excel maestro `Photo/DK`; banca histórica sólo seed reconciliado | ficha, credencial, avatar y depósito de préstamo | RPC self/auditadas; writer final service-only | identidad/foto; cuenta enmascarada opcional; celular confirmado requerido | PII bancaria privada | `public.affiliates`; `affiliate_files/private_assets/Storage`; `affiliate_bank_accounts`; snapshot privado opcional | mismas autoridades; `loan_request_deposit_snapshots` sólo evidencia inmutable | SUPABASE_ACTIVE / ADR-081/085 | Banco/tarjeta/CLABE opcionales para request; si se proporcionan deben ser completos y propios |
