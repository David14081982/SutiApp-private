...
| 26 | Préstamos | `FinancialLegacyRepository` + RPC Auth + Edge + estado in-memory | conjunto préstamos | apertura personalizada, simulación RPC sobre snapshot, historial/Admin | RPC Auth sólo interactiva; Edge service-only abre/confirma; legacy posterior | filtro de perfil + resolver certificado único; timeline/queries separados | Google authority + snapshot derivado TTL 15m | Google reglas + Supabase perfil/política; snapshot/RPC no autoritativos | mismas autoridades vigentes | GOOGLE_LEGACY + ADR-043/064 SNAPSHOT RPC | HIGH; 0 Google/0 Edge interactivo, revalidación Google final |
...
| 29 | Solicitudes/seguimiento | `operationsStore` → `ProgramRequestRepository.listHistory()`; Admin desktop → metadata queue + detail lazy | `program_requests` después de ADR-038; tablas Marketplace previas y Google financiero permanecen históricos/legacy sin nuevas altas | Mi Historial/Tracking y Admin Solicitudes | RPC `update_program_request`; RLS self/company/Admin; auditoría trigger | timeline factual derivado de `created_at`, `updated_at` y estado; documentos bajo demanda | procesamiento financiero posterior permanece Google legacy | Supabase `program_requests` para intención inicial | misma autoridad; sin store o request duplicado | SUPABASE_ACTIVE | Historial refleja la misma fila y estado; responsable/asignación y batch no existen en el contrato actual |
