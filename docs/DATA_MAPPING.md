...
| 29 | Solicitudes/seguimiento | `operationsStore` → `ProgramRequestRepository.listHistory()`; Admin desktop → metadata queue + detail lazy | `program_requests` después de ADR-038; `program_request_admin_events` sólo para decisiones Admin; tablas Marketplace previas y Google financiero permanecen históricos/legacy sin nuevas altas | Mi Historial/Tracking y Admin Solicitudes | RPC general `update_program_request`; Admin financiero usa `record_program_request_admin_action` y aprobación Edge service-only | timeline factual desde solicitud + eventos; `request_documents` históricos y expediente vigente bajo demanda/separado | procesamiento financiero posterior permanece Google legacy | Supabase `program_requests` para intención inicial/estado y bitácora append-only para decisiones | mismas autoridades; sin store, notas mezcladas o request duplicado | SUPABASE_ACTIVE / ADR-076 | Historial refleja la misma solicitud; Admin financiero no reescribe nota del solicitante y no reconstruye adjuntos históricos |
