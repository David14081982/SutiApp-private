...
| 32 | Visibilidad catálogo financiero | Admin Catálogo de Finanzas → `AdminCutoverRepository` → `finance_catalog_presentation` → `finCatStore` → `FinancieraScreen` | `finance_catalog_presentation` para presentación; estructura/rutas en código; elegibilidad en autoridad financiera separada | configuración global autenticada | upsert Admin existente con `workflow.write` | enabled, sort_order, label/description overrides | elegibilidad se aplica después y sólo restringe | ninguna copia/fallback; Membresías explícitamente separada | policy SELECT authenticated; RLS forzada; anon sin grant | SUPABASE_ACTIVE / ADR-083 LIVE+BROWSER PASS | oculto domina elegibilidad; sección vacía omitida; focus/visibility refresh sin polling |
