...
- **INV-213:** La frontera de escritura inicial es A:AG. AH y columnas posteriores están excluidas por
...
- **INV-214:** La cola de entrega pertenece al backend, es durable y no concede escrituras directas al
...
- **INV-215:** Etapas, fechas y motivos provienen del snapshot/tracking/eventos persistidos. Los cambios
...
INV-212 a INV-215 sustituyen exclusivamente las restricciones V1 incompatibles de INV-065/068/071/072/074/079
...
- **INV-069:** La foto de perfil runtime se resuelve exclusivamente por `affiliate.id → affiliate_files(profile_photo, Photo/DK, is_current_profile_photo=true) → private_assets/private-assets`; la URL firmada y su caché en memoria son derivados, se aíslan por principal y nunca se sustituyen con Glide, `DATA`, mock o almacenamiento del navegador. H-PROFILE-PHOTO-EDIT-001 conserva versiones anteriores y documentos; el writer self-only audita el cambio, serializa por afiliado y no acepta un destinatario desde el cliente.
