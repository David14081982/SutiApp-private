...
- Adversarial Chrome: un cambio posterior aborta inmediatamente la cotización anterior. Si la última selección no alcanza el gateway, expira a 6 segundos o Supabase devuelve un fallo explícito de red/transporte, el cliente repite `loanSessionQuote` hasta cinco veces contra el mismo snapshot personalizado, con 500 ms entre intentos; no llama `loanSessionOpen` ni Google. Los errores financieros/no-transitorios y el quinto fallo conservan el estado manual controlado.
