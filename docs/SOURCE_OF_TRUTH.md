...
`sold` es un estado independiente de modalidad, precio y `enabled`. Una fila habilitada y vendida sigue siendo visible, pero ninguna capa puede abrir simulador, solicitar cotización, contactar para adquirir o insertar un `program_request`. `sold_at` y `sold_by` describen el estado vigente; `admin_audit_log` conserva antes/después cuando Admin vende o reactiva. Al quitar Vendido, `commercial_mode` permanece intacto.

Los 35 productos Casa fueron auditados y clasificados `DIRECT_CONTACT`: conservan sus 35 precios y procedencia, y reutilizan el contacto institucional vigente de `Suti Casa` cuando el producto no tiene `contact_url_raw`. Los otros 100 productos conservaron su semántica anterior: 80 `PAYROLL_FIXED` y 20 `PAYROLL_QUOTE`. No existe copia en Marketplace, caché local, mock ni segundo catálogo.
...
ADR-096 alinea el contrato final con ADR-092: una cuenta elegida está completa con Banco + (Tarjeta válida OR CLABE válida), y el instrumento alternativo puede permanecer `NULL`. `affiliate_bank_accounts` sigue siendo el maestro; `loan_request_deposit_snapshots` es sólo evidencia inmutable de la solicitud. `get_request_submission_backend_contract()` expone metadata de compatibilidad para despliegue, no datos de usuario ni una nueva autoridad. El writer service-only y la unicidad `(affiliate_id,idempotency_key)` conservan alta atómica e idempotente.
...
La autoridad runtime es `public.affiliate_files` con `file_key=profile_photo`, `source_column=Photo`, columna `DK`, relación a `private_assets` y objeto en el bucket privado `private-assets`. `AffiliateRepository.getProfilePhoto()` es el único lector frontend; TopBar, Perfil, Credencial y Admin autorizado consumen su proyección en memoria. El importador MASTER ASSET EVACUATION es el escritor histórico. H-PROFILE-PHOTO-EDIT-001, autorizada por el propietario el 2026-09-16, agrega `is_current_profile_photo` (único por afiliado) y el writer `set_self_profile_photo`, que deriva al propietario autenticado, rechaza impersonación, conserva archivos/versiones anteriores y registra auditoría. El frontend invalida la caché y relee la misma autoridad después de guardar. El nuevo frontend requiere la migración `20260916000100`; no utiliza fallback al lector histórico.
...
|---|---|---|---|---|---|---|---|
| Afiliados / identidad y expediente actual | RESOLVED — Supabase `public.affiliates`, inicializada por H-004 desde el Excel maestro reconciliado 947/947 | Supabase `public.affiliates` | `AffiliateRepository`; `AffiliateAuth`; Admin Afiliados; `financial-legacy` consume sólo el contexto financiero actual | Importador H-004; seed exact-hash de categoría/sindicato una sola vez; `claim_affiliate_identity()` limitado; Admin por `affiliates.write` + RPC auditada | ADR-043 + ADR-071 + ADR-093; archive migration APPLIED / VERIFIED | Sí, puente por `numero_control` | Batch `82179501-f85b-50c3-b7ae-7f6998852163`: 947/947, 931 categorías, 770 sindicatos, 16/177 NULL, 0 inferencias y 0 mismatches. Altas Admin usan `record_origin=ADMIN_AFFILIATES` sin coordenadas históricas. El archivo reversible se modela en esta misma tabla, conserva ID/control/Auth/historia y bloquea identidad efectiva; “Eliminados” nunca es una segunda autoridad. Excel SHA-256 `F4BA18ABE82B148ED65737DB16074303627F96D37FA6F9F025E0A10649BD9591` queda sólo como seed/procedencia, nunca fallback ni sync. |
| Autenticación de afiliados | Supabase Auth + vínculo nullable `public.affiliates.auth_user_id`; el email elegible procede sólo de `historical_email_normalized` | Mismo | `AffiliateAuth`, cliente Supabase y `AffiliateRepository` | Preflight mínimo `get_affiliate_activation_status`; `signInWithOtp`; `claim_affiliate_identity`; procesos administrativos autorizados | Corrección productiva publicada; correo/callback reales PASS; cierre completo `BLOCKED_CUSTOM_SMTP` | Sí | No existe identidad paralela. El preflight no devuelve PII ni selecciona emails ambiguos. Site URL/redirects están corregidos; proveedor default entrega, pero 2 correos/hora impiden cerrar password/login/recovery en la certificación. |
...
| Catálogos maestros por programa | RESOLVED por subdominio autorizado — Supabase `program_catalog_items` + `program_catalog_item_assets` + Storage | Mismo | `ProgramCatalogRepository`; Finanzas, programa, cards, detalle Claude y Admin Programas · Productos | Importador H-DATA-CUTOVER-001 para histórico; Admin mediante RPC con `program_catalog.write`; solicitudes iniciales por `program_requests` sólo para modalidades financieras | ACTIVE — 135 filas + 268 vínculos habilitados; 65 precios preservados: 30 fijos y 35 Casa de contacto directo; Cirugías habilitada como programa vacío | Sí: cálculos y procesamiento financiero posterior permanecen legacy | Incluye los 134 históricos (Farma 50, Tours 17, Auto 3, Renta 1, Casa 35, Solar 4, Aires 16, Puertas 3, Cómputo 1, Donativos 1 y Terrenos 3) más el target técnico vigente de Préstamo. Modalidades vigentes: 80 `PAYROLL_FIXED`, 20 `PAYROLL_QUOTE`, 35 `DIRECT_CONTACT`; vendido inicial 0. Cirugías conserva cero filas hasta que Admin registre un producto real mediante `create_first_cirugias_program_catalog_item`; esa primera alta y las posteriores usan `ADMIN_PROGRAM_CATALOG` sin fingir hoja, ordinal o hash. Histórico conserva procedencia; `source_payload` no se concede al browser y Marketplace/Rifas son autoridades separadas. |
...
| Copy editorial de Suti Inversión | RESOLVED — Supabase `public.investment_screen_copy` (25 llaves fijas por migración) | Mismo | Pantalla `Mi Financiera → Invertir` y Admin → Finanzas → Suti Inversión | Admin con `workflow.write`; `authenticated` recibe `select,update` y nunca `insert`/`delete` | ADR-111 ACTIVE (enmienda acotada de ADR-070) | No | Solo texto editorial: encabezado, «Cómo funciona», «Tu respaldo» y aviso legal. `RATE`, `MIN`, `MAX`, `STEP`, `TERMS`, la fórmula y todas las etiquetas de la calculadora, gráfica, pie y CTA permanecen fijas en `screens-inversion.jsx` y no son administrables. La tabla no almacena ningún número que la simulación consuma. Sin respaldo en código, `DATA`, mock ni `localStorage`: si la carga falla la pantalla ofrece reintentar y no inventa texto. La inversión operativa sigue en legacy protegido. |
...
`voting_live_state` es la autoridad única de qué pregunta está al aire (y de la última que lo estuvo) por consulta; lo escribe sólo `set_voting_active_question` y lo mantienen coherente `save_voting_consultation` y `voting_consultation_action` al archivar preguntas o consultas. El navegador lo lee como señal Realtime y siempre recarga el estado por RPC; no guarda copia persistente.

El padrón convocado deja de ser dato capturado: `voting_electorate(audience)` lo deriva de `affiliates` (activos; registrados = con cuenta vinculada; segmento por códigos de sindicato/categoría y cargo; lista = correos de la audiencia). Resultados en vivo: `get_voting_live` con permiso de resultados. Migración `20260916000200`; recovery versionado restaura las funciones exactas del punto de restauración `voting_restore_private` sin borrar consultas, preguntas, votos ni bitácora.
