...
- Implementación: `LiveSeal` lee `useVisualBranding().branding.institutional_seal_url` (misma autoridad `app_settings.institutional_seal_asset` que `SutiSeal`), sin copia local ni fallback: si el sello no carga, no se pinta decoración. Filtro `brightness(0) invert(1)` y opacidad .12 para conservar la marca de agua blanca del diseño; el PNG publicado tiene fondo transparente.
- Hallazgo (preexistente desde 3a1b5ff, 2026-08-29): `visual-content.js` publica el estado completo sin `brandingPhase`, que queda en `error` aunque la identidad visual se cargó. `SutiSeal` sólo pinta con `loaded`, así que el sello no aparece en Inicio tras la carga completa (verificado en producción: 3 instancias `error` sin imagen). La primera publicación del ajuste (`voting-live-20260917-002`, con `SutiSeal`) quedó sin marca de agua por esa causa; `voting-live-20260917-003` lee la URL directamente. La corrección del módulo compartido se autorizó y publicó en H-SUTIAPP-BRANDING-SEAL-PHASE-001.
- Verificación: prueba aislada que reproduce el estado de producción (`brandingPhase` en `error` con la identidad cargada) y usa la imagen real del sello (`VOTING_SEAL_PREVIEW`); app real local contra Supabase productivo con la imagen del sello cargada (`integrated-local-sello.json`); verificación productiva (`production-sello.json`).
