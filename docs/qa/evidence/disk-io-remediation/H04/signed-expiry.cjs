...
async function main(){
 ...
 try{
  ...
  a.save('signed-expiry-'+(process.argv[2]||'before'),{at:new Date().toISOString(),status:'PASS',initial_http:first.status,expired_http:expired.status,expired_token_rejected:true,error_code:String(body.error||body.code||'').replace(/[^a-z0-9_ -]/gi,'').slice(0,80),ttl_seconds:2,urls_logged:0});
