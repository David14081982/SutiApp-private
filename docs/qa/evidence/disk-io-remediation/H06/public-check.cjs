...
const release='C:/tmp/sutiapp-h06-release-20260907',base='https://david14081982.github.io/SutiApp-private/';
...
(async()=>{
 ...
 for(const f of [...files,'index.html','app/supabase-config.js']){
  const expected=f==='app/supabase-config.js'?fs.readFileSync(fs.readFileSync('C:/tmp/sutiapp-h06-current-site.txt','utf8').trim()+'/'+f):cp.execFileSync('git',['show',commit+':'+(f==='index.html'?'SutiApp.html':f)],{cwd:release,maxBuffer:15000000});
  const response=await fetch(base+f+'?h06='+commit,{headers:{'Cache-Control':'no-cache'},signal:AbortSignal.timeout(20000)}),actual=Buffer.from(await response.arrayBuffer());
