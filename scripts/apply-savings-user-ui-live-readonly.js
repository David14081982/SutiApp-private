...
const migrationId = '20260902000300_savings_user_ui_live_readonly';
...
async function management(values, query) {
  ...
  const response = await fetch(`https://api.supabase.com/v1/projects/${ref}/database/query`, {
    method: 'POST',
    headers: { Authorization: 'Bearer ' + values.SUPABASE_ACCESS_TOKEN, 'Content-Type': 'application/json', 'User-Agent': 'SutiApp-SavingsLiveReadonly/1.0' },
...
const checks = `do $verify$ declare v_def text; begin
  ...
  if position('GOOGLE_LEGACY_AUTHORITY' in v_def)=0 or position('SHADOW_MIRROR' in v_def)=0 or position('legacy_reported_balance' in v_def)=0 then raise exception 'RPC_PROVENANCE_CONTRACT_MISSING'; end if;
...
async function main() {
  ...
  const state = await management(values, "select to_regprocedure('public.get_self_savings_live_readonly()') is not null applied");
  const applied = Boolean(state[0] && state[0].applied);
  let mode;
  if (process.argv.includes('--apply')) {
    if (!applied) await management(values, migration);
    await management(values, 'begin;' + checks + 'rollback;');
    mode = applied ? 'VERIFY_APPLIED' : 'APPLIED';
  } else if (applied) {
    await management(values, 'begin;' + checks + 'rollback;');
    mode = 'VERIFY_APPLIED';
  } else {
    const recoveryCheck = "do $recovery_verify$ begin if to_regprocedure('public.get_self_savings_live_readonly()') is not null then raise exception 'RECOVERY_FAILED'; end if; end $recovery_verify$;";
    await management(values, 'begin;' + body(migration) + checks + body(recovery) + recoveryCheck + 'rollback;');
