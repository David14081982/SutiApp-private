...
if(!envPath||!sourcePath||!['bundle','deploy','status','backup'].includes(mode))throw new Error('USAGE: env source [bundle|deploy|status|backup] [backup-path]');
...
(async()=>{
  ...
  if(!['financial-criteria-admin','financial-legacy'].includes(slug))throw new Error('UNEXPECTED_FUNCTION_SLUG');
  if(mode==='backup'){
