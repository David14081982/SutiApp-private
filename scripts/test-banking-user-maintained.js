...
assert.match(repo,/const digits = \(value\) => String\(value \|\| ''\)\.replace\(\/\\D\/g, ''\)/);assert.doesNotMatch(repo,/parseInt|parseFloat|Number\(row\.(?:clabe|account_number|card_number)/);assert.match(repo,/setPrimary/);assert.match(repo,/data_status,incomplete_fields,source_kind/);
