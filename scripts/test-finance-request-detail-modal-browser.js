...
async function main() {
  ...
  try {
    ...
    assert.equal(await readonly.locator('.finwb-primary').count(), 0); await readonly.getByRole('button', { name: 'Siguiente solicitud', exact: true }).click(); await readonly.waitForFunction(() => document.querySelector('dialog h2')?.textContent === 'Carlos Pérez'); await readonly.close();
