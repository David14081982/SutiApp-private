'use strict';
// Isolated contract tests. Executes the unchanged Google mapper; no network/writes.
...
const request={id:'00000000-0000-4000-8000-000000000001',numero_control:'QA-ISOLATED',program_id:'membership',
  ...
  terms_accepted:true,financial_submission_snapshot:{contract_version:'MEMBERSHIP_PAYMENT_V1',financialResult:financial}};
...
// Regression invariant: all shared mapper code is byte-identical to the audited baseline.
assert.equal(read('supabase/functions/financial-legacy/request-google-sync.js'),fs.readFileSync('C:/tmp/sutiapp-membership-payment-20260910/before/supabase/functions/financial-legacy/request-google-sync.js','utf8'));
...
const before=fs.readFileSync('C:/tmp/sutiapp-membership-payment-20260910/before/app/program-request-repository.js','utf8');
...
const ui=read('app/screens-membership-application.jsx'),original=fs.readFileSync('C:/tmp/sutiapp-membership-payment-20260910/before/app/screens-membership-application.jsx','utf8');
...
(async()=>{
  ...
  const result={status:'PASS',googleColumnsEtoI:Array.from(row.slice(4,9)),doubleCharge:false,legacyRequestsUnchanged:true,
    sharedRepositoryNonMembershipIdentical:true,googleMapperIdentical:true,uiCssIdentical:true,quoteAuthorityAndErrors:true,externalWrites:0};
