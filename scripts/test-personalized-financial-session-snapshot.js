...
assert.match(edge,/const \[rawRules, policy\] = await Promise\.all\(\[readCriteriaRules\(privileged\), readTermPolicy\(userClient\)\]\)/);
assert.match(edge,/const rules = await filterSavingsLoanRules\(privileged, rawRules, context\.affiliateId\)/);
...
assert.match(edge,/currentRules = await filterSavingsLoanRules\(privileged, await readCriteriaRules\(privileged\), context\.affiliateId\)/);
