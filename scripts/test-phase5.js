...
must(store.includes('ProgramRequestRepository.listHistory()')&&!store.includes('MarketplaceRepository.listRequests()')&&!store.includes('MarketplaceRepository.listQuotes()'),'safe unified Supabase operation reader missing');
...
must(store.includes('ProgramRequestRepository.listHistory()')&&bundle.includes('/* @@file operations-store.jsx */'),'unified request reader/bundle mismatch');
