...
require('./verification-helpers').assertPwaVersionSync(process.cwd());const financialAsset = html.match(/app\/financial-legacy-repository\.js\?v=\d+/)?.[0];
assert(financialAsset,'financial repository cache contract missing');
assert(read('sw.js').includes(financialAsset),'financial repository cache version differs from HTML');
