...
assert.match(membership,/requirements\('membership',offering\.id\)/);assert.match(membership,/Promise\.allSettled/);assert.match(membership,/UnifiedDocumentPhase/);assert.match(membership,/phase==='ready'/);
...
assert.match(read('app/screens-documentos.jsx'),/Adjuntar archivo/);assert.match(read('app/screens-documentos.jsx'),/Tomar foto/);assert.match(read('app/screens-documentos.jsx'),/capture','environment/);assert.match(workflow,/p_source:source\.toUpperCase\(\)/);
