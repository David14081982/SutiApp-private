...
async function main(){
 ...
 try{
  ...
  for(const f of ['app/vendor/react-18.3.1/react.production.min.js','app/vendor/react-dom-18.3.1/react-dom.production.min.js','app/savings-recorded-history.jsx','app/savings-withdrawal-list.jsx','app/savings-review-admin.jsx'])await page.addScriptTag({content:fs.readFileSync(f,'utf8')});
  await page.evaluate(()=>{
  ...
  await page.getByText('Corregir datos principales',{exact:true}).click();await page.getByLabel('Saldo del archivo original',{exact:true}).fill('600');
  ...
  await page.getByRole('button',{name:'Descuentos',exact:true}).click();await page.getByText('Corregir descuentos por fecha',{exact:true}).click();await page.getByLabel('2026-01-15 · Descuento registrado',{exact:true}).fill('0');
  await page.getByRole('button',{name:'Revisar y guardar',exact:true}).click();await page.getByRole('button',{name:'Revisar antes de guardar',exact:true}).click();assert.match(await page.getByRole('region',{name:'Confirmar revisión'}).innerText(),/Sin dato.*0\.00/);
  await page.getByRole('button',{name:'Volver a editar',exact:true}).click();await page.getByRole('button',{name:'Descuentos',exact:true}).click();await page.getByText('Corregir descuentos por fecha',{exact:true}).click();await page.getByLabel('2026-01-15 · Descuento registrado',{exact:true}).fill('');
  ...
  assert.equal(await page.getByRole('button',{name:'Revisar antes de guardar',exact:true}).count(),0);assert.equal(await page.getByLabel('Saldo del archivo original',{exact:true}).count(),0);assert.deepEqual(errors,[]);
