...
async function main(){
 ...
 try{for(const width of (process.env.SAVINGS_CERTIFICATION_VIEWPORTS||'320,430,1440').split(',').map(Number)){
  ...
  assert((await page.locator('.svp-kpis').innerText()).includes('ahorrando al d'+String.fromCharCode(237)+'a'));
  ...
  const geometry=await page.evaluate(()=>({summary:document.querySelector('.svp-kpis').getBoundingClientRect().bottom,tabs:document.querySelector('[role=tablist]').getBoundingClientRect().top,overflow:document.documentElement.scrollWidth-innerWidth}));assert(geometry.summary<=geometry.tabs);assert(geometry.overflow<=1);
