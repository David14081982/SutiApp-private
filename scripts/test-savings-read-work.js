...
(async()=>{
 ...
 await test('successful savings write invalidates before and after',async()=>{const h=harness(),r=h.window.SavingsRepository,s=h.window.savingsStore;await s.loadSelf();await r.releaseHold('isolated-hold', 'isolated test');assert.equal(s.state().self,null);await s.loadSelf();assert.equal(h.calls.at(-1).params.p_known_version,null);});
