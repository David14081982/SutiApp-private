...
const savingsReadWork = read('supabase/migrations/20260907000500_savings_read_work.sql');
assert.ok(repo.includes("rpc('get_self_savings_if_changed'") && savingsReadWork.includes("'data',public.get_self_savings_live_readonly()") && correction.includes('v_balance_evidence') && correction.includes("#>>'{legacy_reported_balance,value}'"), 'certified Q reader delegation/correction missing');
