...
async function main() {
  ...
  try {
    ...
    const snapshot = () => evaluate(`(()=>{const root=document.querySelector('[data-savings-screen]'),total=document.querySelector('[data-savings-total]'),capital=document.querySelector('[data-savings-capital]'),yieldValue=document.querySelector('[data-savings-yield]');return{screen:Boolean(root),authority:root&&root.dataset.savingsAuthority,cutover:root&&root.dataset.savingsCutover,total:total&&Number(total.dataset.savingsTotal),capital:capital&&Number(capital.dataset.savingsCapital),yield:yieldValue&&Number(yieldValue.dataset.savingsYield),years:document.querySelectorAll('[data-savings-year]').length,upcoming:document.querySelectorAll('[data-savings-upcoming] .sav-next').length,history:document.querySelectorAll('[data-savings-history] .sav-tx').length,actions:document.querySelectorAll('[data-savings-action]').length,sections:['Saldo actual total','Detalle por año','Mi inscripción','Próximos descuentos','Historial'].every(text=>root.innerText.includes(text)),horizontal:document.documentElement.scrollWidth<=document.documentElement.clientWidth&&root.scrollWidth<=root.clientWidth,scrollable:document.querySelector('.sav-scroll').scrollHeight>document.querySelector('.sav-scroll').clientHeight};})()`);
    const initial = await snapshot();
    if (!(initial.screen && initial.authority === 'SHADOW' && initial.cutover === 'NOT_CUTOVER' && initial.total === 48315.2 && initial.capital === 43000 && initial.yield === 5315.2 && initial.years === 2 && initial.upcoming === 2 && initial.history === 2 && initial.actions === 3 && initial.sections && initial.horizontal && initial.scrollable)) throw new Error('SAVINGS_VISUAL_CONTRACT_FAILED ' + JSON.stringify(initial));
