...
const old=f=>cp.execFileSync('git',['show','2aa05b9:'+f],{cwd:root,encoding:'utf8',maxBuffer:32*1024*1024});
...
function shell(source,version){
 ...
 return '<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><link rel="stylesheet" href="app/text-size.css?v='+ (version==='old'?243:244)+'"><main data-version="'+version+'" data-text-size="normal"><span id="result"></span></main><script src="app/text-size-preferences.js?v='+version+'"></script><script>'+bootstrap+'\ntry { const size=TextSizePreferences.fromUser({user_metadata:{sutiapp_text_size:"small"}});document.querySelector("main").dataset.textSize=size;document.getElementById("result").textContent=size;} catch(e){document.getElementById("result").textContent=e.message;}</script>';
}
(async()=>{
 ...
 const server=http.createServer((req,res)=>{
  ...
  if(mode==='failed-install'&&u.pathname==='/app/text-size.css'&&u.search==='?v=244'){res.writeHead(503);return res.end('isolated precache failure');}
 ...
 try{
  ...
  assert((await page.evaluate(()=>caches.keys())).includes('sutiapp-v190'));
  mode='failed-install';
  ...
  assert((await page.evaluate(()=>caches.keys())).includes('sutiapp-v190'),'failed install keeps previous cache');
  ...
  await page.waitForFunction(async()=>{const keys=await caches.keys();return keys.includes('sutiapp-v191')&&!keys.includes('sutiapp-v190');});
