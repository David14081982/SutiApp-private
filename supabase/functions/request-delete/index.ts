...
async function google(payload:Record<string,unknown>){
  const tokenResponse=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:new URLSearchParams({client_id:Deno.env.get('GOOGLE_REQUEST_SYNC_OAUTH_CLIENT_ID')||'',client_secret:Deno.env.get('GOOGLE_REQUEST_SYNC_OAUTH_CLIENT_SECRET')||'',refresh_token:Deno.env.get('GOOGLE_REQUEST_SYNC_OAUTH_REFRESH_TOKEN')||'',grant_type:'refresh_token',scope:'https://www.googleapis.com/auth/drive.file https://www.googleapis.com/auth/script.webapp.deploy'}),signal:AbortSignal.timeout(15000)});
