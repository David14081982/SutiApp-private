...
-- Prevent an Admin publication from assigning the same real context to two
-- enabled request workflows. Request insertion already fails closed on an
-- ambiguity; this moves the rejection to the configuration boundary.
...
begin
  ...
  if exists(
    select 1 from public.operational_workflows other
    where other.id<>p_workflow_id and other.enabled and other.workflow_type='request' and other.service_keys&&v_keys
  ) then raise exception 'WORKFLOW_SERVICE_CONFLICT' using errcode='22023'; end if;
...
comment on function public.validate_operational_workflow_configuration(uuid) is 'Validates publishable request workflow configuration, including unique enabled context assignment.';
