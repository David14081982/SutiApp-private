...
begin
  ...
  select jsonb_build_object(
    ...
    'financial_submission_snapshot',case
      when r.financial_submission_snapshot is null then null
      when r.financial_submission_snapshot->>'contract_version'='PROGRAM_PRODUCT_PAYMENT_V1' then jsonb_build_object(
        'contract_version',r.financial_submission_snapshot->'contract_version',
        'confirmed_at',r.financial_submission_snapshot->'confirmed_at',
        'product',r.financial_submission_snapshot->'product',
        'price_source',r.financial_submission_snapshot->'price_source',
        'authorized_price',r.financial_submission_snapshot->'authorized_price',
        'quote_request_id',r.financial_submission_snapshot->'quote_request_id',
        'down_payment',r.financial_submission_snapshot->'down_payment',
        'financed_amount',r.financial_submission_snapshot->'financed_amount',
        'term',r.financial_submission_snapshot->'term',
        'financialResult',r.financial_submission_snapshot->'financialResult',
        'payment_schedule',r.financial_submission_snapshot->'payment_schedule'
      )
      else jsonb_build_object(
        'confirmed_at',r.financial_submission_snapshot->'confirmed_at',
        'financialResult',r.financial_submission_snapshot->'financialResult'
      ) end,
    'financial_approval_snapshot',case
      when r.financial_approval_snapshot is null then null
      when r.financial_approval_snapshot->>'contract_version'='PROGRAM_PRODUCT_PAYMENT_V1' then jsonb_build_object(
        'contract_version',r.financial_approval_snapshot->'contract_version',
        'approved_at',r.financial_approval_snapshot->'approved_at',
        'product',r.financial_approval_snapshot->'product',
        'price_source',r.financial_approval_snapshot->'price_source',
        'authorized_price',r.financial_approval_snapshot->'authorized_price',
        'down_payment',r.financial_approval_snapshot->'down_payment',
        'financed_amount',r.financial_approval_snapshot->'financed_amount',
        'term',r.financial_approval_snapshot->'term',
        'financialResult',r.financial_approval_snapshot->'financialResult',
        'payment_schedule',r.financial_approval_snapshot->'payment_schedule'
      )
      else jsonb_build_object('financialResult',r.financial_approval_snapshot->'financialResult') end,
...
comment on column public.program_requests.financial_approval_snapshot is
  'Immutable approval contract. Loan approvals may include a Google export; program product payment approvals are Supabase-only.';
comment on function public.get_admin_financial_request_detail(uuid) is
  'Least-privilege Admin financial detail. Product-payment requests expose their immutable price, down payment, financing and schedule without signature or unrestricted snapshots.';
