...
create or replace function public.get_self_savings_live_readonly()
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_actions jsonb;
  v_balance_evidence public.savings_legacy_evidence%rowtype;
  v_legacy_reported_balance numeric;
begin
  ...
  select e.* into v_balance_evidence from public.savings_legacy_evidence e
    where e.participant_id=v_participant.id and e.source_sheet='Ahorro' and e.record_type='LEGACY_REPORTED_BALANCE'
    order by e.source_row desc limit 1;

  v_legacy_reported_balance:=coalesce(
    v_participant.legacy_reported_balance,
    v_balance_evidence.numeric_value,
    nullif(v_balance_evidence.raw_payload#>>'{legacy_reported_balance,value}','')::numeric
  );
  ...
  return jsonb_build_object(
    ...
    'balances',jsonb_build_object(
      'legacy_reported_balance_Q',v_legacy_reported_balance,
      'total',v_legacy_reported_balance,
...
comment on function public.get_self_savings_live_readonly() is
  'Self-only read model over the certified Savings SHADOW mirror. Q is resolved from the participant projection or its certified balance evidence; no canonical ledger or yield calculation is used.';
