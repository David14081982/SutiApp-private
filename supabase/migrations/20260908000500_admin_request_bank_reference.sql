...
begin
  ...
  select jsonb_strip_nulls(jsonb_build_object(
    ...
    'deposit_reference',case when not public.has_admin_permission('bank_accounts.read') then
      jsonb_build_object('status','forbidden')
    else coalesce((select case when d.source_bank_account_id is null then
        jsonb_build_object('status','not_selected')
      else jsonb_build_object('status','available','bank_name',d.bank_name,
        'account_holder',d.account_holder,'card_number',d.card_number,'clabe',d.clabe) end
      from public.loan_request_deposit_snapshots d where d.request_id=r.id and d.affiliate_id=r.affiliate_id),
      jsonb_build_object('status','not_recorded')) end,
