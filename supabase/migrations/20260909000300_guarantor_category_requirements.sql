begin;
-- H-GUARANTOR-CATEGORY-001: owner-authorized conditional requirements. No data rewrites.
do $guard$ begin
  if md5(pg_get_functiondef('public.resolve_effective_document_requirements(text,text)'::regprocedure))<> '86019940037d00451789fb7362318826' then raise exception 'GUARANTOR_FUNCTION_DRIFT: resolve_effective_document_requirements'; end if;
  if md5(pg_get_functiondef('public.get_document_requirement_impact(text,text)'::regprocedure))<> '29236b775257c29409a4507e0bee34d9' then raise exception 'GUARANTOR_FUNCTION_DRIFT: get_document_requirement_impact'; end if;
  if md5(pg_get_functiondef('public.capture_document_requirements_snapshot()'::regprocedure))<> '0c9fbdf7138e7e2bae06c73506bbd0b6' then raise exception 'GUARANTOR_FUNCTION_DRIFT: capture_document_requirements_snapshot'; end if;
  if md5(pg_get_functiondef('public.create_validated_financial_program_request(uuid,uuid,uuid,uuid,text,text,uuid,uuid[],uuid,numeric,integer,text,integer,jsonb)'::regprocedure))<> 'f6aa8e78687abf16a81d06030659304e' then raise exception 'GUARANTOR_FUNCTION_DRIFT: create_validated_financial_program_request'; end if;
  if md5(pg_get_functiondef('public.create_validated_financial_program_request_bank_required(uuid,uuid,uuid,uuid,text,text,uuid,uuid[],uuid,numeric,integer,text,integer,jsonb)'::regprocedure))<> '1def6bb403b85a21f5b02eb6658756d8' then raise exception 'GUARANTOR_FUNCTION_DRIFT: create_validated_financial_program_request_bank_required'; end if;
  if md5(pg_get_functiondef('public.create_validated_financial_program_request_v1_engine(uuid,uuid,uuid,uuid,text,text,uuid,uuid[],uuid,numeric,integer,text,integer,jsonb)'::regprocedure))<> 'a034d98f78069ae3c81fa2b47eb16985' then raise exception 'GUARANTOR_FUNCTION_DRIFT: create_validated_financial_program_request_v1_engine'; end if;
  if md5(pg_get_functiondef('public.create_validated_program_product_payment_request(uuid,uuid,uuid,uuid,text,text,uuid,uuid[],uuid,numeric,integer,integer,date)'::regprocedure))<> '81727c5e371213c310c2e6a1b7a3cb4c' then raise exception 'GUARANTOR_FUNCTION_DRIFT: create_validated_program_product_payment_request'; end if;
  if md5(pg_get_functiondef('public.create_membership_request(uuid,uuid[],text,text,text,uuid,uuid)'::regprocedure))<> '91e0c2d1767da501233bac7666364310' then raise exception 'GUARANTOR_FUNCTION_DRIFT: create_membership_request'; end if;
  if md5(pg_get_functiondef('public.finalize_program_request_context(uuid,uuid,uuid[])'::regprocedure))<> '7e535aa5e5f0f339a00d31cebd5d2e14' then raise exception 'GUARANTOR_FUNCTION_DRIFT: finalize_program_request_context'; end if;
end $guard$;

create function public.required_guarantor_document_codes(p_affiliate_id uuid)
returns text[] language sql stable security definer set search_path=''
as $$
  select case when a.financial_employee_category_code in ('SUPLENTES_VARIABLES','SUPLENTES_FIJOS')
    then array['guarantor_ine_front','guarantor_ine_back','guarantor_photo']::text[]
    else array[]::text[] end
  from public.affiliates a where a.id=p_affiliate_id
$$;
revoke all on function public.required_guarantor_document_codes(uuid) from public,anon,authenticated;
grant execute on function public.required_guarantor_document_codes(uuid) to service_role;
CREATE OR REPLACE FUNCTION public.resolve_affiliate_document_requirements(p_scope_type text, p_scope_key text, p_affiliate_id uuid)
 ...
 SET search_path TO ''
AS $function$
declare v_scope jsonb;v_type text;v_key text;v_parent_type text;v_parent_key text;v_guarantor_codes text[];
begin
  ...
  if p_affiliate_id is not null then
    v_guarantor_codes:=public.required_guarantor_document_codes(p_affiliate_id);
    if v_guarantor_codes is null then raise exception 'AFFILIATE_REQUIRED' using errcode='42501'; end if;
    if cardinality(v_guarantor_codes)>0 and
      (select count(*) from public.document_types d where d.enabled and d.code=any(v_guarantor_codes))<>3 then
      raise exception 'GUARANTOR_DOCUMENT_CATALOG_UNAVAILABLE' using errcode='55000';
    end if;
  end if;
  return query
  with candidates as(
  ...
  select result.* from (
    select c.id,v_type,v_key,c.document_type_id,
      case when d.code=any(v_guarantor_codes) then true else c.required end,
      c.allow_verified_reuse,c.sort_order,c.is_inherited,c.scope_type,c.scope_key,
      d.code,d.label,d.description,d.icon,d.accepted_mime_types,d.camera_allowed,d.file_upload_allowed,d.max_file_size_bytes
    from chosen c join public.document_types d on d.id=c.document_type_id and d.enabled
    where p_affiliate_id is null
      or d.code not in ('guarantor_ine_front','guarantor_ine_back','guarantor_photo')
      or d.code=any(v_guarantor_codes)
    union all
    -- The category policy supplements scopes with no configured guarantor rows.
    -- NULL requirement_id distinguishes the policy from a persisted configuration row.
    select null::uuid,v_type,v_key,d.id,true,true,d.sort_order,false,v_type,v_key,
      d.code,d.label,d.description,d.icon,d.accepted_mime_types,d.camera_allowed,d.file_upload_allowed,d.max_file_size_bytes
    from public.document_types d
    where d.enabled and d.code=any(v_guarantor_codes)
      and not exists(select 1 from chosen c where c.document_type_id=d.id)
  ) result order by 7,(select ordering.sort_order from public.document_types ordering where ordering.id=result.document_type_id),12;
end $function$;

revoke all on function public.resolve_affiliate_document_requirements(text,text,uuid) from public,anon,authenticated;
grant execute on function public.resolve_affiliate_document_requirements(text,text,uuid) to service_role;
...
AS $function$
begin
  return query select * from public.resolve_affiliate_document_requirements(
    p_scope_type,p_scope_key,public.get_effective_affiliate_id());
end $function$;
...
begin
  ...
  select count(*) into v_requirements from public.resolve_affiliate_document_requirements(v_type,v_key,null);
...
begin
  ...
  from public.resolve_affiliate_document_requirements(v_type,v_key,new.affiliate_id) r;
  return new;
end $function$;
...
begin
  ...
  select count(*) into v_missing from public.resolve_affiliate_document_requirements('PROGRAM','prestamo',p_affiliate_id) r
   where r.required and not exists(
...
begin
  ...
  from public.resolve_affiliate_document_requirements('PROGRAM','prestamo',p_affiliate_id) requirement
...
AS $function$ declare v_affiliate public.affiliates%rowtype;v_row public.program_requests%rowtype;v_missing integer; begin
  ...
  select count(*) into v_missing from public.resolve_affiliate_document_requirements('MEMBERSHIP',p_membership_offering_id::text,v_affiliate.id) r where r.required and not exists(
...
AS $function$ declare v_row public.program_requests%rowtype;v_missing integer; begin
  ...
  select count(*) into v_missing from public.resolve_affiliate_document_requirements(
      case when v_row.membership_offering_id is not null then 'MEMBERSHIP'
        when v_row.program_id='prestamo' or v_row.financial_submission_snapshot->>'contract_version'='PROGRAM_PRODUCT_PAYMENT_V1' then 'PROGRAM'
        when v_row.product_id is not null then 'PRODUCT' else 'PROGRAM' end,
      case when v_row.membership_offering_id is not null then v_row.membership_offering_id::text
        when v_row.program_id='prestamo' or v_row.financial_submission_snapshot->>'contract_version'='PROGRAM_PRODUCT_PAYMENT_V1' then 'prestamo'
        when v_row.product_id is not null then v_row.product_id::text else v_row.program_item_id::text end,
      v_row.affiliate_id) r where r.required and not exists(
