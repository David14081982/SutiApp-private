begin;
-- Read-only administrative totals. Imported reference rows and their writers stay intact.
-- The renamed reader is a reversible, inaccessible copy of the previous contract.
alter function public.get_admin_savings_panel(text,text,text,integer,integer) rename to savings_panel_before_current_summary;
revoke all on function public.savings_panel_before_current_summary(text,text,text,integer,integer) from public,anon,authenticated,service_role;
alter function public.get_admin_savings_panel_detail(uuid,integer,integer) rename to savings_detail_before_current_summary;
alter function public.get_admin_savings_panel_neighbor(uuid,text,text,text,integer) rename to savings_neighbor_before_current_summary;
revoke all on function public.savings_detail_before_current_summary(uuid,integer,integer),public.savings_neighbor_before_current_summary(uuid,text,text,text,integer) from public,anon,authenticated,service_role;

create function public.savings_admin_current_summary() returns jsonb
language sql stable security definer set search_path='' as $$
 with imported as materialized (
  ...
  from public.savings_panel_people() a join public.savings_review_records r on r.id=(a->>'id')::uuid
...
$$;
revoke all on function public.savings_admin_current_summary() from public,anon,authenticated,service_role;

create function public.savings_admin_filtered_people(p_tab text,p_search text,p_filter text,p_summary jsonb default null) returns setof jsonb
language sql stable security definer set search_path='' as $$
 select a from jsonb_array_elements(coalesce(p_summary,public.savings_admin_current_summary())->'rows') a
 where a->>'native'='false'
  and (coalesce(p_search,'')='' or position(lower(p_search) in lower(coalesce(a->>'nombre','')))>0 or position(p_search in coalesce(a->>'folio',''))>0)
  and (p_tab<>'revision' or ((a->>'needs_review')::boolean or a->>'status'='RESOLVED'))
  and (p_tab<>'cobranza' or a->>'legacy_zero_recorded'='true')
  and (p_filter='todos' or p_filter='pendientes' and a->>'status'<>'RESOLVED' or p_filter='resueltas' and a->>'status'='RESOLVED'
   or p_filter='pausado' and (a->>'estado'='pausado' or a->>'zero_recorded'='true') or a->>'estado'=p_filter);
$$;
revoke all on function public.savings_admin_filtered_people(text,text,text,jsonb) from public,anon,authenticated,service_role;
...
declare result jsonb; summary jsonb; rows jsonb; balance_total numeric; mode text; filtered_total integer;
begin
 -- The preserved reader performs permission, scope and duplicate-baseline checks first.
 result:=public.savings_panel_before_current_summary(p_tab,p_search,p_filter,p_offset,p_limit);
 summary:=public.savings_admin_current_summary();
...
revoke all on function public.get_admin_savings_panel(text,text,text,integer,integer) from public,anon,authenticated,service_role;
grant execute on function public.get_admin_savings_panel(text,text,text,integer,integer) to authenticated;

create function public.get_admin_savings_panel_detail(p_record_id uuid,p_history_limit integer default 6,p_history_offset integer default 0) returns jsonb
language plpgsql stable security definer set search_path='' as $$
declare result jsonb; person jsonb;
begin
 result:=public.savings_detail_before_current_summary(p_record_id,p_history_limit,p_history_offset);
 select a into person from public.savings_admin_filtered_people('padron','','todos')a where a->>'id'=p_record_id::text;
 return result||jsonb_build_object('source_person',result->'person','person',person,'publication_mode',(select mode from public.savings_publication_state where id),'as_of',public.savings_operation_today());
end $$;
...
language plpgsql stable security definer set search_path='' as $$
declare summary jsonb; anchor jsonb; result jsonb;
begin
 ...
 if p_record_id is null or p_direction is null or p_direction not in (-1,1) or p_tab is null or p_tab not in ('padron','revision','cobranza')
  or p_filter is null or p_filter not in ('todos','ahorrando','pausado','baja','pendientes','resueltas') or length(coalesce(p_search,''))>200 then raise exception 'SAVINGS_PANEL_INVALID';end if;
 ...
 summary:=public.savings_admin_current_summary();
 select a into anchor from jsonb_array_elements(summary->'rows') a where a->>'native'='false' and a->>'id'=p_record_id::text;
 if anchor is null then raise exception 'SAVINGS_PANEL_NOT_FOUND';end if;
 with numbered as (
  select a,row_number() over(order by a->>'nombre',a->>'folio',(a->>'source_row')::int,a->>'id') n,count(*) over() total
  from public.savings_admin_filtered_people(p_tab,p_search,p_filter,summary)a
 ), candidates as (
  ...
  else row(a->>'nombre',a->>'folio' is null,coalesce(a->>'folio',''),(a->>'source_row')::int,a->>'id')<
   row(anchor->>'nombre',anchor->>'folio' is null,coalesce(anchor->>'folio',''),(anchor->>'source_row')::int,anchor->>'id') end
 ) select jsonb_build_object('id',a->>'id','has_prev',n>1,'has_next',n<total) into result from candidates
  order by case when p_direction=1 then n end,case when p_direction=-1 then n end desc limit 1;
 return result;
end $$;
revoke all on function public.get_admin_savings_panel_detail(uuid,integer,integer),public.get_admin_savings_panel_neighbor(uuid,text,text,text,integer) from public,anon,authenticated,service_role;
