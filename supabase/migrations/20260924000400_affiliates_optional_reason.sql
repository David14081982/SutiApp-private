...
do $guard$ begin if (select pg_get_constraintdef(oid) from pg_constraint where conrelid='public.affiliate_admin_events'::regclass and conname='affiliate_admin_events_reason_check') is distinct from 'CHECK (((length(btrim(reason)) >= 8) AND (length(btrim(reason)) <= 500)))' then raise exception 'OPTIONAL_REASON_CONSTRAINT_DRIFT:affiliate_admin_events_reason_check'; end if; end $guard$;

alter table public.affiliate_admin_events drop constraint affiliate_admin_events_reason_check;

alter table public.affiliate_admin_events add constraint affiliate_admin_events_reason_check CHECK (((length(btrim(reason)) >= 0) AND (length(btrim(reason)) <= 500)));

do $guard$ begin if (select pg_get_constraintdef(oid) from pg_constraint where conrelid='public.affiliate_profile_audit_log'::regclass and conname='affiliate_profile_audit_log_reason_check') is distinct from 'CHECK (((length(btrim(reason)) >= 8) AND (length(btrim(reason)) <= 500)))' then raise exception 'OPTIONAL_REASON_CONSTRAINT_DRIFT:affiliate_profile_audit_log_reason_check'; end if; end $guard$;

alter table public.affiliate_profile_audit_log drop constraint affiliate_profile_audit_log_reason_check;

alter table public.affiliate_profile_audit_log add constraint affiliate_profile_audit_log_reason_check CHECK (((length(btrim(reason)) >= 0) AND (length(btrim(reason)) <= 500)));
...
do $guard$ begin if (select pg_get_constraintdef(oid) from pg_constraint where conrelid='public.impersonation_sessions'::regclass and conname='impersonation_reason_check') is distinct from 'CHECK (((char_length(btrim(reason)) >= 8) AND (char_length(btrim(reason)) <= 500)))' then raise exception 'OPTIONAL_REASON_CONSTRAINT_DRIFT:impersonation_reason_check'; end if; end $guard$;

alter table public.impersonation_sessions drop constraint impersonation_reason_check;

alter table public.impersonation_sessions add constraint impersonation_reason_check CHECK (((char_length(btrim(reason)) >= 0) AND (char_length(btrim(reason)) <= 500)));
