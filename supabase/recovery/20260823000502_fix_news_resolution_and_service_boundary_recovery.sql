...
begin
  ...
  select count(*) into v_count from auth.users u where lower(u.email)=lower(btrim(p_email)) and u.email_confirmed_at is not null;
...
begin
  ...
  foreach v_action in array v_actions loop
    if not public.has_section_action('news',v_action) then raise exception 'NEWS_ACTION_DENIED: %',v_action using errcode='42501'; end if;
  end loop;
  if tg_op='DELETE' and old.record_origin<>'ADMIN_PHASE2' then raise exception 'NEWS_HISTORICAL_DELETE_DENIED' using errcode='42501'; end if;
