...
-- Disable the capability while preserving its immutable audit history.
delete from public.admin_role_permissions
where permission in('financial_criteria.visibility.read','financial_criteria.visibility.write');
update public.admin_assignments a
set permissions=(select coalesce(array_agg(permission order by permission),'{}') from public.admin_role_permissions where role_id=a.role_id),updated_at=now();
...
revoke select on public.financial_criteria_visibility_audit from authenticated;
drop policy if exists financial_criteria_visibility_audit_read on public.financial_criteria_visibility_audit;

-- The audit table is intentionally retained. Removing the Edge deployment or
-- rotating the server secret stops Google writes immediately.
commit;
