...
create function public.get_financial_shadow_runtime_rules()
...
comment on function public.financial_runtime_rules_for_batch(uuid) is
  'Internal no-grant projection shared by authoritative runtime and certified shadow canary.';
comment on function public.get_financial_shadow_runtime_rules() is
  'Service-only deployment canary restored for cutover recovery.';
