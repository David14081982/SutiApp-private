-- Recovery for 20260827001000_loan_snapshot_quote_key_share_lock.
-- Restores the FOR SHARE row lock by replaying the original 20260826000100
-- definition of resolve_current_loan_snapshot_quote. No data is touched: the
-- change is a row-lock mode on a read-only select, so recovery is unconditional
-- and reversible.
