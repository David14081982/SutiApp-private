...
do $$
begin
  if to_regclass('public.affiliate_csv_auth_link_repair_batches') is not null
    and exists(select 1 from public.affiliate_csv_auth_link_repair_batches) then
    raise exception 'RECOVERY_BLOCKED_REPAIR_HISTORY_EXISTS';
  end if;
end;
$$;
...
drop function public.apply_affiliate_csv_auth_link_repair(uuid,text,text,integer,integer,integer,integer,integer,integer,jsonb);
drop function public.recover_affiliate_csv_auth_link_repair(uuid);
drop function public.has_certified_affiliate_auth_link(uuid,uuid,text);
drop table public.affiliate_csv_auth_link_repairs;
drop table public.affiliate_csv_auth_link_repair_snapshot;
drop table public.affiliate_csv_auth_link_repair_batches;
