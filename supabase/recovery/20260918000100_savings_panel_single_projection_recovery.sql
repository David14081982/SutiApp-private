begin;
-- Reversal for 20260918000100_savings_panel_single_projection.
-- Restores the three functions exactly as they were before that migration.
drop function if exists public.savings_admin_current_summary(jsonb);
CREATE OR REPLACE FUNCTION public.savings_admin_current_summary()
 RETURNS jsonb
 LANGUAGE sql
...
$function$;
drop function if exists public.savings_panel_before_current_summary(text,text,text,integer,integer,jsonb);

CREATE OR REPLACE FUNCTION public.savings_panel_before_current_summary(p_tab text DEFAULT 'padron'::text, p_search text DEFAULT ''::text, p_filter text DEFAULT 'todos'::text, p_offset integer DEFAULT 0, p_limit integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO ''
