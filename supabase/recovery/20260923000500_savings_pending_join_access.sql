...
begin
 ...
 where signature='public.savings_loan_eligibility(uuid):20260923000500';
 if pg_get_functiondef('public.savings_loan_eligibility(uuid)'::regprocedure)<>prior.installed_definition then
  raise exception 'SAVINGS_PENDING_JOIN_RECOVERY_DRIFT';
 end if;
 execute prior.definition;
end $recover$;
update public.financial_session_snapshots set invalidated_at=now(),invalidation_reason='SAVINGS_PENDING_JOIN_ACCESS_RECOVERY'
